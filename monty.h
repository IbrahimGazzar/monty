#ifndef MONTY_H
#define MONTY_H
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

/**
 * struct stack_s - doubly linked list representation of a stack (or queue)
 * @n: integer
 * @prev: points to the previous element of the stack (or queue)
 * @next: points to the next element of the stack (or queue)
 *
 * Description: doubly linked list node structure
 * for stack, queues, LIFO, FIFO
 */
typedef struct stack_s
{
	int n;
	struct stack_s *prev;
	struct stack_s *next;
} stack_t;
/**
 * struct instruction_s - opcode and its function
 * @opcode: the opcode
 * @f: function to handle the opcode
 *
 * Description: opcode and its function
 * for stack, queues, LIFO, FIFO
 */
typedef struct instruction_s
{
	char *opcode;
	void (*f)(stack_t **stack, unsigned int line_number);
} instruction_t;
/**
 * struct argument_s - hold global variables
 * @fp: the monty file
 * @line: holds the line to be read
 * @line_n: holds the current line number
 * @tokens: array of tokens for the current line
 * @tokenum: number of tokens in tokens
 * @instruct: holds the instruction to be run
 *
 * Description: holds the variables that will be used throughout the
 * entire program
 */
typedef struct argument_s
{
	FILE *fp;
	char *line;
	unsigned int line_n;
	char **tokens;
	int tokenum;
	instruction_t *instruct;
} args_t;
void file_handler(char *filename);
void line_handler();
void tokenizer();
void run_command();
void unknown_instruction();
void malloc_fail();
void free_arg();
void push(stack_t **stack, unsigned int l_num);
void pall(stack_t **stack, unsigned int l_num);
void pint(stack_t **stack, unsigned int l_num);
void pop(stack_t **stack, unsigned int l_num);
void swap(stack_t **stack, unsigned int l_num);
void add(stack_t **stack, unsigned int l_num);
void nop(stack_t **stack, unsigned int l_num);

extern args_t *args;
extern stack_t *stack;

#endif
